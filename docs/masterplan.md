...
- Keep generated projects on `@anmho/authctl@0.1.1` or newer so a fresh install
  exposes `resource-servers upsert`; `service create` should stop before auth
  registration if the installed package regresses.
